...
proc checkRequiredFiles { origin_dir} {
  set status true
  set files [list \
 "[file normalize "$origin_dir/axi_lockbox.v"]"\
 "[file normalize "$origin_dir/diff_io_mgr.v"]"\
 "[file normalize "$origin_dir/dac_splitter.v"]"\
 "[file normalize "$origin_dir/axi_scope.v"]"\
 "[file normalize "$origin_dir/pid_loop.v"]"\
 "[file normalize "$origin_dir/ramp_module.v"]"\
 "[file normalize "$origin_dir/clocks.xdc"]"\
 "[file normalize "$origin_dir/ports.xdc"]"\
  ]
  foreach ifile $files {
...
if { [get_files axi_lockbox.v] == "" } {
  import_files -quiet -fileset sources_1 axi_lockbox.v
}
if { [get_files diff_io_mgr.v] == "" } {
  import_files -quiet -fileset sources_1 diff_io_mgr.v
}
if { [get_files dac_splitter.v] == "" } {
  import_files -quiet -fileset sources_1 dac_splitter.v
}
if { [get_files axi_scope.v] == "" } {
  import_files -quiet -fileset sources_1 axi_scope.v
}
if { [get_files pid_loop.v] == "" } {
  import_files -quiet -fileset sources_1 pid_loop.v
}
if { [get_files ramp_module.v] == "" } {
  import_files -quiet -fileset sources_1 ramp_module.v
}
...
# module references:
# axi_lockbox, axi_scope, dac_splitter, diff_io_mgr
  ...
  set block_name axi_lockbox
  set block_cell_name axi_lockbox_0
  if { [catch {set axi_lockbox_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
  } elseif { $axi_lockbox_0 eq "" } {
  ...
  if { [catch {set axi_scope_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $axi_scope_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  set block_name dac_splitter
  set block_cell_name dac_splitter_0
  if { [catch {set dac_splitter_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $dac_splitter_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  
  # Create instance: diff_io_mgr_0, and set properties
  set block_name diff_io_mgr
  set block_cell_name diff_io_mgr_0
  if { [catch {set diff_io_mgr_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $diff_io_mgr_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [ list \
...
if { [get_property IS_LOCKED [ get_files -norecurse system.bd] ] == 1  } {
  import_files -fileset sources_1 [file normalize "${origin_dir}/system_wrapper.v" ]
} else {
